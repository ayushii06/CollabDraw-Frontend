...
import { elementType, fillStyle, Options, penType, strokeStyle, ToolBar } from "../../../lib/types";
import { createElements } from "./createElement";
import { Offset } from "../../UI/Home";
...
const updateElement = ({ id, x1, y1, x2, y2, tool, options, elements, setElements, setPanOffset }: propsType) => {
      ...
      switch (tool) {
            ...
            case 'circle':
                  ...
                  elementsCopy[id] = createElements({ id, x1, y1, x2, y2, tool, options });
            ...
            case 'text':
                  ...
                  const textWidth = rc.measureText(options.text).width;

                  const textHeight = options.size;


                  elementsCopy[id] = { ...elementsCopy[id], tool, x1: x1, y1: y1, x2: x1 + textWidth, y2: y1 + textHeight, text: options.text, color: options.color, size: options.size };
